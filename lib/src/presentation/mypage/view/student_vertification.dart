...
class _VertificationViewState extends State<VertificationView> with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    return Stack(
                              ...
                              if (!isUploaded)
                                if (widget.userResponse.personalInfo?.verification.isVerificationFailed ?? false)
                                  Column(
                                    children: [
                                      SizedBox(height: SizeConfig.defaultSize * 2,),
                                      Text("학생증 사진이 또렷하지 않아 인증에 실패했어요!", style: TextStyle(
                                          fontSize: SizeConfig.defaultSize * 1.8,
                                        color: Colors.red
                                      )),
                                      SizedBox(height: SizeConfig.defaultSize * 0.3),
                                      Text("이름, 학교, 학번이 모두 나오도록 다시 촬영해보세요!", style: TextStyle(
      ...
      // 로딩 인디게이터
        _isLoading
            ? const MeetProgressIndicatorFullScreen(text: "학생증을 제출하고 있어요!", color: Colors.white, opacity: 0.5,)
