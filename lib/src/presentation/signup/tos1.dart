...
class Tos1 extends StatelessWidget {
  const Tos1({super.key});
...
이 약관은 Frolic (이하 '회사' 라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
10. '멤버십포인트'란 회원이 회사의 유료서비스를 구매할 경우 회사에서 고지한 적립률에 따라 부여되는 포인트 또는 회원이 회사 또는 제휴회사에서 주최하는 이벤트 등 마케딩 활동 등과 관련하여 획득한 포인트를 말합니다. 이 때 구체적인 '멤버십포인트'의 명칭 등에 관하여는 별도의 조항에서 정합니다.
11. '할인쿠폰'은 이용자가 회사의 서비스를 이용하면서 그 대가를 지급하는데 사용하기 위하여 회사가 발행 및 관리하는 지급수단을 말합니다. 
12. 'Point'(은)는 회사가 제공하는 유료서비스를 이용하기 위해 구매하는 인터넷 상의 결제수단으로 현금 1원당 Point 10의 비율로 환산되며, 회원의 아이디(ID)를 기준으로 충전, 결제, 환불 등이 가능합니다. 
13. '콘텐츠'란 정보통신망법의 규정에 따라 정보통신망에서 사용되는 부호 ·문자·음성·음향·이미지 또는 영상 등으로 정보 형태의 글, 사진, 동영상 및 각종 파일과 링크 등을 말합니다.
...
회원정보 및 이용내역에 따른 맞춤 서비스
직접 개발하거나 제휴를 통해 제공되는 서비스
기타 이용자의 편의를 위하여 제공하는 서비스
...
회원은 유료서비스를 구입하고 그에 따른 대금을 결제한 경우나 회사 또는 제휴회사에서 주최하는 이벤트, 추가 적립 서비스 등 마케팅 활동 등과 관련하여, 회사의 포인트 적립 정책에 따라 Points를 부여받게 됩니다.
'할인쿠폰'은 회사가 구매활동 또는 이벤트 참여 등에 따른 보상으로 발행 합니다. 이 때 발행된 쿠폰은 사용하거나 만료되면 소멸되며 현금으로 환급되지 않습니다.
Points 은 현금으로 환급될 수 없습니다. 포인트의 사용기간이 만료되거나 구매 취소 시 또는 이용계약이 종료되면 소멸합니다.
