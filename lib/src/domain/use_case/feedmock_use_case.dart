...
class FeedMockUseCase {
  SurveyDetail mockUpSurveyDetail = SurveyDetail(
      ...
      options: [Option(id: 1, name: '그렇다', headCount: 10), Option(id: 2, name: '아니다', headCount: 25)],
      picked: false,
      pickedOption: 1,
  ...
  Pagination<Survey> mockUpSurveys = Pagination(content: [
    ...
    Survey(
        ...
        options: [Option(id: 3, name: '한다 🙆🏻‍♀️', headCount: 30), Option(id: 4, name: '안 한다🙅🏻‍♀️', headCount: 80)],
        picked: true,
        pickedOption: 3,
        createdAt: DateTime.now(),
  ...
  Pagination<Survey> getSurveys({int page = 0, int size = 10}) { // 오늘의 질문 목록 get (Pagination)
    return mockUpSurveys;
  }

  SurveyDetail getSurvey(int id) { // 오늘의 질문 한 개 get
    return mockUpSurveyDetail;
  }

  void pickOption() { // 선택지 선택 post
    mockUpSurveyDetail = SurveyDetail(
        id: 0,
        question: '여사친에게 겉옷 빌려주는 행동 플러팅일까?',
        options: [Option(id: 1, name: '그렇다', headCount: 50), Option(id: 2, name: '아니다', headCount: 25)],
        picked: false,
        pickedOption: 1,
        createdAt: DateTime.now(),
        latestComment: '여사친한테 겉옷 빌려주면 이게 플러팅이지 뭐얏!!!',
  ...
  void postComment(User userResponse, String content, DateTime createdAt) { // 댓글 추가
    mockUpSurveyDetail.comments.add(
      Comment(id: 0, writer: userResponse, content: content, createdAt: createdAt, likes: 0, liked: false)
    );
    print(Comment(id: 0, writer: userResponse, content: content, createdAt: createdAt, likes: 0, liked: false)
    );
  }
  ...
  void reportComment(int commentId) { // 댓글 삭제
    // TODO : report
  }

  void postLikeComment(int commentId) { // 댓글 좋아요
    // TODO : like
  }
